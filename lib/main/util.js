...
const SERVICE_OPTIONS = { host: LOCALHOST, port: 8080 };
const TIMEOUT = 3600;
...
const connectWhistle = (req, reqSock, callback, options) => {
  connect(options || PROXY_OPTIONS, (err, socket) => {
    // 确保销毁所有连接
    if (err) {
      if (!req._hasError) {
        destroy(req);
...
const proxyRequest = (req, callback, options) => {
...
exports.getWhistleData = (req) => {
  req.method = 'GET';
  delete req.headers['content-length'];
  try {
    return getWhistleData(req);
  } catch (e) {
    return getWhistleData(req);
  }
};

const proxyRequestThunk = (ctx, options) => {
  return new Promise((resolve, reject) => {
    proxyRequest(ctx.req, (err, svrRes) => {
      if (err) {
        return reject(err);
      }
...
exports.passToService = async (ctx) => {
  SERVICE_OPTIONS.port = await startService();
  return proxyRequestThunk(ctx, SERVICE_OPTIONS);
};
