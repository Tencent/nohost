...
module.exports = (options, callback) => {
  ...
  ].concat(oldCustomPluginsPath); // 用于卸载

  isHeadless = /^\$\d+/.test(username);
  const mode = 'multiEnv|disableUpdateTips|keepXFFs|x-forwarded-proto';
  ...
  getPort((port) => {
    proxy = startWhistle({
      ...
      projectPluginsPath,
      customPluginsPath,
      pluginsPath: (options.pluginsPath || []).concat(oldCustomPluginsPath),
