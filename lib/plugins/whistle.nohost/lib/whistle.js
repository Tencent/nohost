...
module.exports = (options, callback) => {
  ...
  const pluginsPath = projectPluginsPath.concat(customPluginsPath);
  ...
  if (process.env.PFORK_MODE === 'bind') {
    pluginsPath.unshift(process.cwd());
  }
  getPort((port) => {
    proxy = startWhistle({
      port,
