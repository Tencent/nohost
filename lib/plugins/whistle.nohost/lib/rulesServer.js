...
module.exports = (server) => {
  ...
  app.use(async (ctx) => {
    ...
    const filter = (getRuleValue(ctx) === 'none' || ctx.get('x-whistle-nohost-hide')) ? 'ignore://htmlAppend' : '';
    if (!account) {
      ctx.body = {
    ...
    if (accountMgr.accountRules && !reqHeaders[WHISTLE_RULE_VALUE]) {
      reqHeaders[WHISTLE_RULE_VALUE] = encodeURIComponent(accountMgr.accountRules);
    }
