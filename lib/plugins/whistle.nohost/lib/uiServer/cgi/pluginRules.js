...
module.exports = (ctx) => {
  ...
  if (baseUrl) {
    if (curBaseUrl !== baseUrl || curPatterns !== patterns) {
      ...
      let ignorePatterns = [];
      const injectRule = [`internal-proxy://${baseUrl} ignore://-proxy|-host ${INJECT_RULE} enable://clientId|multiClient|safeHtml`];
      const normalRule = [`internal-proxy://${baseUrl} ignore://-proxy|-host enable://clientId|multiClient`];
      curRules = [];
      ...
      curRules.unshift(`internal-proxy://${baseUrl} ignore://rule|html|-proxy|-host enable://hide|proxyHost|clientId|multiClient ${domainList.join(' ')}`);
