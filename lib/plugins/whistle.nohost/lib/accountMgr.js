...
const parseEntryPatterns = (str) => {
  const priPatterns = [];
  const pubPatterns = [];
  str.trim().split(/\s+/).forEach((pattern) => {
    if (pattern[0] === '_' && pattern[1]) {
      priPatterns.push(pattern.substring(1));
    } else if (pattern[0] !== '#' && !priPatterns.includes(pattern)) {
      pubPatterns.push(pattern);
    }
  });
  const pluginRules = [];
  const entryRules = priPatterns.map((pattern) => {
    pluginRules.push(`${pattern} ${proxyServer}`);
    return `${pattern} whistle.nohost://none`;
  }).concat(pubPatterns.map((pattern) => {
    pluginRules.push(`${pattern}  ${proxyServer} whistle.nohostenv://`);
    return `${pattern} whistle.nohost://`;
  }));
  if (certsDomain) {
    certsDomain.forEach((domain) => {
      if (!priPatterns.includes(domain) && !pubPatterns.includes(domain)) {
        pluginRules.push(`${domain} ${proxyServer}`);
        entryRules.push(`${domain} whistle.nohost://`);
      }
    });
  }
  return {
...
class AccountMgr {
  ...
  parseRules() {
    ...
    const {
      ...
      pubPatterns,
      pluginRules,
    } = parseEntryPatterns(this.entryPatterns);
    this.allEntryRules = `${entryRules}\n${this.entryRules}`;
    this.priPatterns = priPatterns;
    this.pubPatterns = pubPatterns;
  ...
  setBaseUrl(url) {
    baseUrl = `http://${url}/`;
    const server = `internal-proxy://${url}`;
...
module.exports = (options) => {
  const { baseUrl: url } = options.data;
  if (url) {
    baseUrl = `http://${url}/`;
    proxyServer = `internal-proxy://${url}`;
  }
