...
const parseRule = (rules, defaultRules, accountRules, testRules, varRegExp, keyValueMap) => {
  const headers = {};
...
class AccountMgr {
  ...
  parseRules() {
    ...
    let accountRules = getStringProperty('accountRules');
    ...
    accountRules = trimRules(accountRules);
    testRules = trimRules(testRules);
    const { varRegExp, keyValueMap } = this;
    list.forEach((env) => {
      this.parsedMap[env.key] = parseRule(env.rules, accountRules, defaultRules, testRules, varRegExp, keyValueMap);
    });
  }
