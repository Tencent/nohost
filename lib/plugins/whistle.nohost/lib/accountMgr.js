...
function getRemoteRules(rulesText) {
  if (!rulesText) {
    return rulesText;
  }
  return rulesText.replace(REMOTE_RULES_RE, (_, rulesUrl) => {
...
class AccountMgr {
  constructor() {
    ...
    this.parseRules();
    this.parsePatterns();
    httpMgr.addChangeListener(() => this.parseRules(true));
  }
  ...
  parseRules(updateRemoteRules) {
    ...
    if (updateRemoteRules || this.defaultRules !== defaultRules || this.testRules !== testRules) {
      this.defaultRules = defaultRules;
      this.testRules = testRules;
      httpMgr.clean();
      defaultRules = getRemoteRules(defaultRules);
      testRules = getRemoteRules(testRules);
    }
    this.parsedMap = {};
    accountMap = {};
