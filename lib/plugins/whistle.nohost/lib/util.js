...
const isWorker = name => name && WORKER_RE.test(name);

exports.isWorker = isWorker;

exports.getWorkerName = (name) => {
  if (!isWorker(name)) {
    return;
  }
  const len = name.length;
  name = name.substring(len > 15 ? len - 15 : 1);
  return `$${parseInt(name, 10) % WORKER_NUM}`;
};
